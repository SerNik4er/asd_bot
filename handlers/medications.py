...
from keyboards import get_main_keyboard, get_medications_keyboard
...
async def add_start_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not name or not dosage:
        await update.message.reply_text("❌ Ошибка: данные потеряны. Попробуйте заново.")
...
async def list_medications(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for med in meds:
        ...
        name_escaped = escape_markdown(name)
        dosage_escaped = escape_markdown(dosage)
        text += f"• *{name_escaped}* ({dosage_escaped})\n   с {start_date}\n"
...
async def report_medication_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not logs:
        await query.message.reply_text(
            f"📊 *Отчёт по лекарству {escape_markdown(name)} ({escape_markdown(dosage)})*\n\n"
    ...
    report = f"📊 *Отчёт по лекарству {escape_markdown(name)} ({escape_markdown(dosage)})*\n\n"
